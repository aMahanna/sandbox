...
(*
  Question 1 Answers
    ...
    (i) Variables visible in the body of F  

        - x (declared on line 14)
        - a (declared on line 18)
        - y (declared on line 19)
        - z (declared on line 20)


    (ii) Variables visible in the body of G 

        - y (declared on line 15)
        - z (declared on line 16)
        - a (declared on line 24)
        - x (declared on line 25)
        - w (declared on line 26)
    
    (iii) Variables visible in the body of H 
        
        - y (declared on line 15)
        - x (declared on line 25) 
        - w (declared on line 26)
        - a (declared on line 28) 
        - b (declared on line 29)
        - z (declared on line 30)
...
let main = 
  ...
  let f (u : int) = 
    ...
    let y = 10 in
    let z = 12 in 
    let resultF = (2 * u) - (x + y + z + a) (* ...code for f... *)
    in Printf.printf "Variables of F: [x = %d], [y = %d], [z = %d], [a = %d],[resultF = %d]\n%!" x y z a resultF
  ...
  in let g (u : int) = 
       ...
       let h (v : int) = 
         ...
         let b = 22 in
         let z = 24 in 
         let resultH = (2 * u * v) - (a + b + z + x + w + y) (* ...code for h... *)
         in (Printf.printf "Variables of H: [a = %d], [b = %d], [z = %d], [x = %d], [w = %d], [y = %d],[resultH = %d] \n%!" a b z x w y resultH; resultH)
       
       in let resultG = 2 * (h 1) (* ...code for g... *)
       
       in (Printf.printf "Variables of G: [a = %d], [x = %d], [w = %d], [y = %d], [z = %d], [resultG = %d] \n%!" a x w y z resultG; resultG)
  
in f (2 *(g 1)) (* ...code for main (outer) block... *)
...
(* Answers for Question 2 have been added in a seperate PDF file *)
...
(* In "pass-by-value-result", also called "call-by-value-result" and
   ...
   More specifically, suppose a function f with
   ...
   parameter u that is initialized to the R-value of v. 
  ...
  Within the
   ...
   not change the local value of y.  
...
(* In this case, since the parameter is passed  
    by value-result, it means that the final value of x will be 1. 
    This is because y' is given a new location distinct from x, so the
    assignment to x does not change the local value of y'.
*)
let x = ref 0
let p (y' : int ref) : unit =
  let y = !y' in
    let y = 1 in 
      (x := 0; y' := y; Printf.printf "[y' = %d], [x = %d]" !y' !x)
let result = p x
let _ = x

(* In this case, since the parameter is passed  
    by reference, it means that x & y' are aliases,
      and so the assignment to x would change the value of y' to 0. 
*)
let x = ref 0
let p (y' : int ref) : unit = 
  let _ = y' := 1 in 
...
(* In this case, since the parameter is passed  
    by value, it means that the assignment to y' in the body of p does
      not change the global variable x, and the final value of x would
        also be 0.
*)
let x = ref 0
...
let power_tr (a: int) (b:int) = 
  let rec power' (x:int) (a:int) (b:int) =
    if b <= 0 then x
    else if b = 1 then x * a
    else power' (x * a) a (b - 1)
  in power' 1 a b
