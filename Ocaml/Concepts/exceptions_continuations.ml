...
(* Question 1 Answer: 
  ...
  1) "call (f1 11)"
    - x = 11
    - function return = "call (f1 9)"

  2) "call (f1 9)"
    - x = 9
    - function return = "call (f1 7)"

  3) "call (f1 7)"
    - x = 7
    - function return = "call (f1 5)"

  4) "call (f1 5)"
    - x = 5
    - function return = "call (f1 3)"
  
  5) "call (f1 3)"
    - x = 3
    - function return = "call (f1 1)"

  6) "call (f1 1)"
    - x = 1
    - function return = Raises Exception "Odd"
    - No handler found in call, must pop activation record of function call & look at previous AR

  7) "(f1 1)" is Popped

  8) Check for handler in "call (f1 3)"
    - x = 3
    - No handler is found, must pop activation record of function call & look at previous AR 

  7) "(f1 3)" is Popped

  8) Check for handler in "call (f1 5)"
    - x = 5
    - Handler for exception "Odd" is FOUND, return complement of current x (-x)
    
    ---> Final Output: f1 returns -5
*)
...
let _ =
  ...
  (* QUESTION 2. 
      ...
      --> In certain cases, the Exception implementation is more efficient 
        that the first, because it will sometimes never have to reach lines 133 or 134.

        This is because of the current match case established on line 130, which will
        raise an exception and terminate the function call and go straight to the handler return value. 

        This way, we avoid having to compute certain values and avoid making an additional function call.
        Keep in mind that this may only occur sometimes. 
  *)
...
(* Question 3 Exception Implementation *)
exception Q3Exception
let f (x : int) : int = 
  try (1 + (if x < 0 then raise Q3Exception else (x/2)))
  with Q3Exception -> 0
